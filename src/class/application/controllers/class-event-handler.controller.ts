...
import { ApplicationStatus, ClassApplicationUpdatedEventPattern, ClassApplicationUpdatedEventPayload, ClassSessionCreatedEventPattern, ClassSessionCreatedEventPayload, MultipleClassSessionsCreatedEventPattern, MultipleClassSessionsCreatedEventPayload } from '@tutorify/shared';
...
export class ClassEventHandlerController {
  ...
  async handleClassSessionCreated(payload: ClassSessionCreatedEventPayload) {
    ...
    const classToInsertClassSession = await this.classService.getClassById(classId);
    const isValidClass = !!classToInsertClassSession;
    const isValidTutor = classToInsertClassSession.tutorId === createSessionTutorId;
    this.eventDispatcher.dispatchClassSessionClassVerified(classSessionId, isValidClass);
    this.eventDispatcher.dispatchClassSessionTutorVerified(classSessionId, isValidTutor);
  }

  @EventPattern(new MultipleClassSessionsCreatedEventPattern())
  async handleMultipleClassSessionsCreated(payload: MultipleClassSessionsCreatedEventPayload) {
